...
RUN_ID="${RUN_ID:-act_calvin_a_clean_bs64_200k}"
...
PYTHONPATH="${LEROBOT_REPO}/src" python "${LEROBOT_REPO}/src/lerobot/scripts/train.py" \
  --dataset.repo_id=local/calvin-task-A-train-lerobot \
  --dataset.root="${LEROBOT_DATA_HOME}/local/calvin-task-A-train-lerobot" \
  --policy.type=act \
  --policy.device=cuda \
